...
const getDeliveries = async (req, res) => {
    const delivery = await Ninja.find({}).sort({createdAt: -1})
...
const getDelivery = async (req, res) => {
    ...
    const delivery = await Ninja.findById({_id: id})
    if(!delivery){
        return res.status(400).json({error: "No Delivery Found"})
    }
    res.status(200).json(delivery)
...
const createDelivery = async (req, res) => {
    ...
    try {
        ...
        const delivery = await Ninja.create({ 
            deliveries, 
            amount, 
...
const deleteDelivery = async (req, res) => {
    ...
    const delivery = await Ninja.findByIdAndDelete({_id: id})
    if(!delivery){
        return res.status(400).json({error: "No Delivery Found"})
    }
...
const updateDelivery = async (req, res) => {
    try {
        ...
        const delivery = await Ninja.findByIdAndUpdate(id, {
            deliveries,
            amount,
            paidAmount: paidAmount || 0,
            balance,
...
const getDeliverySummary = async (req, res) => {
    try {
        const result = await Ninja.aggregate([
          {
            $group: {
              _id: null,
...
const getMonthly = async (req, res) => {
    ...
    try {
        ...
        const expense = await Ninja.aggregate([
            {
                $match: {
                    date: {
                        $gte: startDate,
...
const getDaily = async (req, res) => {
    ...
    try {
        ...
        const expense = await Ninja.find({
