...
const getDeliveries = async (req, res) => {
    const delivery = await Hunger.find({}).sort({createdAt: -1})
...
const getDelivery = async (req, res) => {
    ...
    const delivery = await Hunger.findById({_id: id})
    if(!delivery){
        return res.status(400).json({error: "No Delivery Found"})
    }
    res.status(200).json(delivery)
...
const createDelivery = async (req, res) => {
    ...
    try {
        ...
        const delivery = await Hunger.create({ 
            deliveries, 
            amount, 
...
const deleteDelivery = async (req, res) => {
    ...
    const delivery = await Hunger.findByIdAndDelete({_id: id})
    if(!delivery){
        return res.status(400).json({error: "No Delivery Found"})
    }
...
const updateDelivery = async (req, res) => {
    try {
        ...
        const delivery = await Hunger.findByIdAndUpdate(id, {
            deliveries,
            amount,
            paidAmount: paidAmount || 0,
            balance,
...
const getDeliverySummary = async (req, res) => {
    try {
        const result = await Hunger.aggregate([
          {
            $group: {
              _id: null,
...
const getMonthly = async (req, res) => {
    ...
    try {
        ...
        const expense = await Hunger.aggregate([
            {
                $match: {
                    date: {
                        $gte: startDate,
...
const getDaily = async (req, res) => {
    ...
    try {
        ...
        const expense = await Hunger.find({
