...
const getDeliveries = async (req, res) => {
    const delivery = await Jahez.find({}).sort({createdAt: -1})
...
const getDelivery = async (req, res) => {
    ...
    const delivery = await Jahez.findById({_id: id})
    if(!delivery){
        return res.status(400).json({error: "No Delivery Found"})
    }
    res.status(200).json(delivery)
...
const createDelivery = async (req, res) => {
    ...
    try {
        ...
        const delivery = await Jahez.create({ 
            deliveries, 
            amount, 
...
const deleteDelivery = async (req, res) => {
    ...
    const delivery = await Jahez.findByIdAndDelete({_id: id})
    if(!delivery){
        return res.status(400).json({error: "No Delivery Found"})
    }
...
const updateDelivery = async (req, res) => {
    try {
        ...
        const delivery = await Jahez.findByIdAndUpdate(id, {
            deliveries,
            amount,
            paidAmount: paidAmount || 0,
            balance,
...
const getDeliverySummary = async (req, res) => {
    try {
        const result = await Jahez.aggregate([
          {
            $group: {
              _id: null,
...
const getMonthly = async (req, res) => {
    ...
    try {
        ...
        const expense = await Jahez.aggregate([
            {
                $match: {
                    date: {
                        $gte: startDate,
...
const getDaily = async (req, res) => {
    ...
    try {
        ...
        const expense = await Jahez.find({
